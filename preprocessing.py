...
@dataclass
class Config:
    outdir: str = "preprocessing_outputs"
    ...
    # Loss weights
    lambda_who: float = 5.0   # up-weighted: WHO monthly fit is primary objective
    lambda_year: float = 0.01  # down-weighted: yearly constraint should not crush monthly peaks
...
def load_wiki_monthly() -> pd.Series:
    ...
    w = w.copy()
    w["date"] = parse_monthly_date(w["Month"])
...
def build_design_matrix(
    ...
    # Z-score standardize continuous features (over the full modeling window)
    # Originals (g_fever, g_vaccine, wiki_views) kept in df for output/inspection.
    continuous_cols = ["g_fever", "g_vaccine"]
    if cfg.use_wiki:
        continuous_cols.append("wiki_views")
    ...
    # Build X with intercept + standardized continuous features + month dummies
    df["intercept"] = 1.0
    feature_cols = ["intercept", "g_fever_z", "g_vaccine_z"]
    if cfg.use_wiki:
        feature_cols += ["wiki_views_z"]
    ...
    return df, X, y_who, mask_who, feature_cols, scaler


def compute_data_boundaries(design_df: pd.DataFrame) -> dict:
    """
    Compute the three canonical data boundary timestamps for the Step 1 window.

    Returns a dict with keys:
      who_observed_through      – last YYYY-MM with actual WHO reported cases (y_who non-null)
      external_data_through     – last YYYY-MM where Google Trends (g_fever) is non-null
      nowcast_available_through – same as external_data_through (Step 1 can predict up to here)

    These boundaries are saved into params_step1.json and consumed by the Step 2
    notebook to correctly separate observed / imputed / nowcast segments.
    """
    # WHO observed endpoint — raw observed only, never imputed/extended
    who_mask = design_df['y_who'].notna() & np.isfinite(design_df['y_who'].astype(float))
    who_months = design_df.loc[who_mask, 'date']
    who_observed_through = who_months.max().strftime('%Y-%m') if not who_months.empty else None

    # External data endpoint — last month with Google Trends available (minimum required feature)
    ext_mask = design_df['g_fever'].notna() & np.isfinite(design_df['g_fever'].astype(float))
    ext_months = design_df.loc[ext_mask, 'date']
    external_data_through = ext_months.max().strftime('%Y-%m') if not ext_months.empty else None

    return {
        'who_observed_through': who_observed_through,
        'external_data_through': external_data_through,
        'nowcast_available_through': external_data_through,
    }
...
# (5) Outputs
def save_params(outdir: Path, feature_cols: List[str], beta_hat: np.ndarray, cfg: Config, scaler: dict, boundaries: dict = None) -> None:
    params = {
        "config": asdict(cfg),
        "features": feature_cols,
        "beta": {name: float(val) for name, val in zip(feature_cols, beta_hat)},
        "target_scale": cfg.target_scale,
        "feature_scaler": scaler,  # mean/std used to standardize continuous features
    }
    if boundaries is not None:
        params["data_boundaries"] = boundaries
    ...
    out = pd.DataFrame(
        {
            ...
            # raw continuous features (for inspection)
            ...
            # standardized versions actually used in the model
            "g_fever_z": design_df["g_fever_z"].astype(float).values if "g_fever_z" in design_df.columns else np.nan,
            "g_vaccine_z": design_df["g_vaccine_z"].astype(float).values if "g_vaccine_z" in design_df.columns else np.nan,
            "wiki_views_z": design_df["wiki_views_z"].astype(float).values if "wiki_views_z" in design_df.columns else np.nan,
...
def main():
    ...
    design_df, X, y_who, mask_who, feature_cols, scaler = build_design_matrix(monthly_wide, cfg)
    ...
    # Compute and save data boundaries
    boundaries = compute_data_boundaries(design_df)

    # Save artifacts
    save_params(outdir, feature_cols, beta_hat, cfg, scaler, boundaries)
    ...
    pred_df.to_csv(outdir / "preprocessed_monthly.csv", index=False)

    plot_who_vs_pred(pred_df, outdir)
    plot_yearly_vs_od(pred_df, yearly_proxy, outdir)

    # Data boundary summary
    print("\nData boundaries (saved to params_step1.json):")
